...
const BMICalculator = ({ weight, height, onBMIChange, onWeightChange, onHeightChange }) => {
  ...
  // Use a ref to track the last BMI value we reported
  // This prevents calling onBMIChange when the value hasn't actually changed
  const lastReportedBMI = useRef(null);

  useEffect(() => {
    const bmi = parseFloat(calculateBMI());
    // Only call onBMIChange if the BMI value actually changed
    if (bmi !== lastReportedBMI.current) {
      lastReportedBMI.current = bmi;
      onBMIChange(bmi);
    }
    // NOTE: onBMIChange is intentionally excluded from deps array.
    // Including it would cause the infinite loop since Dashboard
    // passes it as an inline arrow function creating a new ref each render.
