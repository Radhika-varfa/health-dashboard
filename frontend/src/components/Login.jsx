...
const Login = ({ setToken, setUser }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1. Save to localStorage FIRST so Dashboard can read it synchronously
      localStorage.setItem('token', token);
      localStorage.setItem('user', JSON.stringify(userData));
      ...
      // 3. Navigate immediately — NO setTimeout needed.
      //    localStorage is already set so Dashboard will never see a missing token.
